...
def train_val_test(train_loader, val_loader, test_loader, model, device, PROCESS_BATCH, TEST_MODEL, params,
    ...
    for epoch in range(params['num_epoch']):
        ...
        val_mae, val_mape, val_log = TEST_MODEL(model, val_loader, device, val_edge, val_node, val_A)
        is_best_change = early_stop.append(val_mae)

        if is_best_change:
            print('current mae:', val_mae, 'current mae:', early_stop.best_metric(), 'current log:', val_log)
    ...
    test_mae, test_mape, test_log = TEST_MODEL(model, test_loader, device, test_edge, test_node, test_A)
